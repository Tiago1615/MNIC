...
% Matriz de transición global (P)
P = [1,   0,   0,   0,   0,   0;   % Nodo A (Absorbente)
...
fprintf('a) Si la rata se coloca inicialmente en la habitación D, ¿cuál es el número esperado de veces que entrará en la habitación C, antes de encontrar comida?: %.2f\n', visitas_D_C);

% (b) Tiempo de absorción desde E
% Sumamos la fila correspondiente a E (índice 4 en idxT)
tiempo_desde_E = sum(N_fund(4, :));
fprintf('b) Si la rata se coloca inicialmente en la habitación E, ¿cuál es el número esperado de habitaciones en las que entrará, antes de entrar en la habitación A o F?: %.2f\n', tiempo_desde_E);
...
prob_C_A = B_prob(2, 1);
fprintf('c) Si la rata se coloca inicialmente en la habitación C, ¿cuál es la probabilidad de que encuentre comida en la habitación A?: %.4f\n', prob_C_A);

% (d) Probabilidad de acabar en F (col 2) desde E (fila 4 de idxT)
prob_E_F = B_prob(4, 2);
fprintf('d) Si la rata se coloca inicialmente en la habitación E, ¿cuál es la probabilidad de que encuentre comida en la habitación F?: %.4f\n', prob_E_F);

% Mostrar matrices calculadas para verificación
disp('------------------------------------------------------------------------------------------------------------------------------------------------------------------------');
disp('Matriz de transición (P):'); disp(P);
